...
def align_mvt(d1, d2, zmin=None, zmax=None):
    r1 = [os.path.relpath(p, d1) for p in d1.rglob("*.pbf")]
    r2 = [os.path.relpath(p, d2) for p in d2.rglob("*.pbf")]
    ...
    if zmin is not None or zmax is not None:
        def filt(p):
            ...
            if zmax is not None and z > zmax:
                return False
            return True
        r1 = [p for p in r1 if filt(p)]
        r2 = [p for p in r2 if filt(p)]

    t1 = {r: load_mvt(d1 / r) for r in r1}
    t2 = {r: load_mvt(d2 / r) for r in r2}
